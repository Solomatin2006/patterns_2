...
class AuthTest {
    ...
    void shouldGetErrorIfBlockedUsers() {
        var blockedUsers = getRegisteredUser("blocked");
        $("[data-test-id='login'] input").setValue(blockedUsers.getLogin());
        $("[data-test-id='password'] input").setValue(blockedUsers.getPassword());
        $("button.button").click();
        $("[data-test-id='error-notification'] .notification__content")
